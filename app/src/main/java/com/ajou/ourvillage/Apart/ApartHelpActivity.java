...
public class ApartHelpActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        RecyclerView recyclerView = findViewById(R.id.apart_help_recyclerview);
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(layoutManager);

        ArrayList<ApartPostItem> dataList = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            dataList.add(new ApartPostItem(R.drawable.ic_launcher_background, "이름임", "2021년 5월 3일 월요일", "옆집 조용히해주시면안될까여", "잠을 못자겠어여.", "5", "3", false));
        }
